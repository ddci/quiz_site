...
var $indicators = $('<ol>')
  ...
  */
  $.each(questions, function(question_index, question) {
    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      var opts = {
        ...
        confirmButtonText: "Наступне запитання",
        html : true,
      ...
      if (correct) {
        ...
        opts = $.extend(opts, {
          title: "Ні,це не так",
          text: (
            "Ні , навіть не наближено до правди!<br/><br/>" +
      ...
      if (last_question) {
        opts.confirmButtonText = "Переглянути результати";
      }
  ...
  var $social = $("<div>")
    ...
    .html('<div id = "social-text">Хей! Даня вважає,що Вам сподобалось, поділись с друзями та даси "відмінно"!</div>')
    .appendTo($results_slide);
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      text = "Вау&mdash;прекрасний результат!";
      break;
    case (ratio > 0.9):
      text = "Гарна робота, ти відповів на більшість питаннь вірно.";
      break;
    case (ratio > 0.60):
      text = "Неплохо, ала ти можеш краще.";
      break;
    case (ratio > 0.5):
      text = "Ок, але тільки половина була вірна&hellip;";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = "Тобі було важко,їди відпочивай";
      break;
    case (ratio === 0):
      text = "Воооооу, ти на усі запитанні відповів невірно,ті що скажений?";
      break;
  }
  return text;
